...
- Label: `Reload suggestions` (optional icon allowed)
...
- If `groqApiKey` is empty, do not fire a request; show an inline key-required hint and keep controls disabled
- POST to `/api/suggest` with `{ transcript: contextWindow, prompt, apiKey }`
...
### Rendering
- Column header title must be exactly `2. LIVE SUGGESTIONS`
- Each batch is rendered as a group:
  - Newest batch: full opacity
  - Older batches: reduced opacity (for example `opacity-40`) and a subtle visual separator
  - Batch footer: `- BATCH N · HH:MM:SS AM/PM -` centered below the 3 cards
- Each suggestion card:
  - Small colored type badge at top-left:
...
- Transcript context is shorter than `suggestContextChars`; send the full transcript
- API key missing; request is blocked client-side and a key-required hint is shown
- User clicks Reload while a request is already in-flight; button is disabled, so this cannot occur
- Groq rate limit or network error; show inline error `Failed to load suggestions. Retrying in 30s.` and do not reset accumulated batches
...
- Missing key guard: with empty `groqApiKey`, reload is disabled and no `/api/suggest` request is fired